...
public class FormController {
	...
	public String binding(Model model, String busFormId) {
		...
		Map<String, String> map = list.stream().collect(Collectors.toMap(Objects::getName, Objects::getSid));
	...
	// ztree 树型结构数据json
	@RequestMapping(value = "/design-json", method = { RequestMethod.GET })
	@ResponseBody
	public String designJson(String sid,Map<String, Object> container) {
		// 查询表单关联业务对象
		Map<String, Object> parames = new HashMap<String, Object>();
		parames.put("busFormId", sid);
		List<formRelation> relation = formrelationService.select(parames);
		...
		if (relation != null && relation.size() > 0) {
			...
			container.put("data", JSON.toJSONString(tree));
			return "success";
		} else {
			// 跳转表单绑定业务界面
			return "";
